...
public class OrderRepositoryImpl extends GenericDaoImpl<Order> implements OrderRepository {
    ...
    public List<Order> getAllBySeller(String email) {
        Query q=entityManager.createQuery("select o from Orders o join o.shoppingCart c join  c.item i join i.product p where p.seller.email=:email");
    ...
    public List<Order> getAllByBuyer(String email) {
        Query q=entityManager.createQuery("select o from Orders o where o.shoppingCart.buyer.email=:email");
        q.setParameter("email",email);
        return q.getResultList();
    }
}
